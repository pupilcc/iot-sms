...
static int64_t s_last_wifi_reconnect_time = 0;  // Track when Wi-Fi last reconnected
...
/*
 * @brief Wi-Fi event handler to track reconnection events
 */
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
                                int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        // Record the time when IP address is obtained
        s_last_wifi_reconnect_time = esp_timer_get_time();
        ESP_LOGI(TAG, "IP address obtained, network ready for MQTT connection");
    }
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        ESP_LOGI(TAG, "MQTT keep-alive: 30s, connection stable");
    ...
    case MQTT_EVENT_ERROR:
        ...
        if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
            ...
            // Improved Wi-Fi and network status diagnostics
            if (!wifi_manager_is_connected()) {
                ESP_LOGW(TAG, "Wi-Fi is disconnected! MQTT connection will resume once Wi-Fi reconnects.");
            } else if (!wifi_manager_has_ip()) {
                ESP_LOGW(TAG, "Wi-Fi connected but IP address not yet assigned. Waiting for DHCP...");
            } else {
                // Calculate time since last Wi-Fi reconnect
                int64_t now = esp_timer_get_time();
                int64_t time_since_reconnect_ms = (now - s_last_wifi_reconnect_time) / 1000;
...
{
    // Register Wi-Fi event handler to track reconnections
    ...
    esp_mqtt_client_config_t mqtt_cfg = {
        ...
        .session.keepalive = 30,                  // Keep-alive interval 30 seconds
